...
function kMostFrequent(nums, k) {
  ...
  for (var i = 0; i < nums.length; i++) {
    if(mapQuest.hasOwnProperty(nums[i])){
      mapQuest[nums[i]] += 1;
    } else {
      mapQuest[nums[i]] = 1;
    }
  ...
  let returnArr = []
  for(var j= 0; j < k; j++){
    let maxVal = Math.max(...mapQuest.values());
    // check the highest value in the object
    console.log(maxVal);
    // add the key to returnarr
    // remove/ignore that pair from the object
  }
}

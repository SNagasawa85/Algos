...
function orderedMultisetUnion(sortedA, sortedB) {
    ...
    while(aIdx < sortedA.length-1 && bIdx < sortedB.length-1){

        if(sortedA[aIdx] < sortedB[bIdx]){
            unionArr.push(sortedA[aIdx]);
            aIdx++;
        }
        if(sortedB[bIdx] < sortedA[aIdx]){
            unionArr.push(sortedB[bIdx]);
            bIdx++; 
        }
        if(sortedA[aIdx] === sortedB[bIdx]){
            unionArr.push(sortedA[aIdx]);
    ...
    if(aIdx >= sortedA.length-1){
        for(var j = bIdx; j<sortedB.length; j++){
            unionArr.push(sortedB[j]);
        }
    }
    if(bIdx >= sortedB.length-1){
        for(var j = aIdx; j< sortedA.length; j++){
            unionArr.push(sortedA[j]);
        }
    }
    return unionArr;
}
